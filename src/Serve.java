...
//	double impatience = 0.25; //impatience = 0.2 (99% leave probability for customer 22 in queue) patience = 1 (99% for customer 5 in queue)
	...
	//impatience = 0.2 (99% leave probability for customer 22 in queue) patience = 1 (99% for customer 5 in queue)
	double impatience = 0.25; 
	...
	public void cleanTable(Server server) throws InterruptedException {
        ...
        if(leftoverCups + leftoverGlasses > 0) {
        	System.out.println(server.name + " is washing " + leftoverCups +" cup(s) and " +
        					   leftoverGlasses + " glass(es).");
        	Thread.sleep(100);
        	System.out.println(server.name + ": finish washing. Added " + leftoverCups +" cup(s) and " + 
        					   leftoverGlasses + " glass(es).");
        }
        else {
        	System.out.println(server.name + " went to table but there is nothing to clean.");
